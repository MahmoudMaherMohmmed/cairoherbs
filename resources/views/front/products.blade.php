            ...
            <div class="row">
                <div class="col-12">
                    <div class="breadcrumb_content">
                        @if(App::isLocale('ar'))
                            <h3>{{ __('website.products') }} {{isset($category)&&$category!=null ? getDefaultValueKey($category->title) : ''}}</h3>
                        @else
                            <h3>{{isset($category)&&$category!=null ? getDefaultValueKey($category->title) : ''}} {{ __('website.products') }}</h3>
                        @endif
                        <ul>
                            <li><a href="{{route('home')}}">{{ __('website.home') }}</a></li>
                            @if(App::isLocale('ar'))
                                <li>{{ __('website.products') }} {{isset($category)&&$category!=null ? getDefaultValueKey($category->title) : ''}}</li>
                            @else
                                <li>{{isset($category)&&$category!=null ? getDefaultValueKey($category->title) : ''}} {{ __('website.products') }}</li>
                            @endif
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="shop_toolbar_wrapper">
                        ...
                        <div class="page_amount">
                            <p>
                                ...
                                –{{ $products->lastItem() }}
                                {{ __('website.of') }} {{ $products->total() }} {{ __('website.results') }}
                            </p>
                        </div>
                    </div>
                                                    ...
                                                    <ul>
                                                        <li class="quick_button">
                                                            <a href="#" data-toggle="modal"
                                                               ...
                                                               title="quick view"
                                                               onclick="modalData('{{$product->id}}', '{{$product->code}}', '{{getDefaultValueKey($product->title)}}', '{{getDefaultValueKey($product->description)}}', '{{Storage::url($product->image)}}', '{{route('product',[$product->slug])}}')">
                                                                <i class="icon-eye"></i>
                                                            </a>
                                                        </li>
                                                    </ul>
